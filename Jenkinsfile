    ...
    stages {
        ...
        stage('Wait for DB Service') {
            steps {
                script {
                    ...
                    sh 'while ! docker-compose -f docker-compose.yml exec db mysqladmin ping -h 127.0.0.1 --silent --password="$MYSQL_ROOT_PASSWORD"; do sleep 5; done'
                }
            }
        }
        ...
        stage('Run Tests') {
            steps {
                ...
                sh 'docker-compose -f docker-compose.yml exec backend ./run-tests.sh'
            }
        }
        ...
        stage('Deploy') {
            steps {
                script {
                    ...
                    sh 'docker-compose -f docker-compose.yml up -d proxy'
                }
            }
        }
    }
